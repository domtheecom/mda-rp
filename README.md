...
<<<<<<< codex/build-custom-fivem-server-scripts-30494x
>>>>>>> main
3. Confirm the `endpoint_add_tcp` and `endpoint_add_udp` values match the port assigned by your host (keep the defaults if txAdmin proxies to 30120).
4. Update the MySQL connection string (or move credentials to environment variables) and run the SQL migrations generated by the scripts automatically (tables are created at runtime).
5. Edit `mda_rolesync/config.lua` to include the real Discord role IDs.
6. Provide the correct asset files for the loading screen (`logo.png`, `background.jpg`, `music.mp3`, etc.).
7. Add approved MLO resources to `resources/[mda_resources]/mda_mlo_loader` and register them with the provided event (`TriggerEvent('mda_mlo_loader:register', 'resourceName')`).
<<<<<<< codex/build-custom-fivem-server-scripts-yu6dsk
=======
=======
...
<<<<<<< codex/build-custom-fivem-server-scripts-30494x
>>>>>>> main
- When enabling pre-installed packs, ensure each actual resource (e.g., `leo_fhp`, `md_graphics_core`) is listed with its own `ensure` line; category folders such as `[LEO]` are not resources by themselves.

## Troubleshooting
- **`Failed to fetch server variables` / CURL error 7**: This indicates txAdmin cannot reach the running FXServer. Verify the `endpoint_add_tcp`/`endpoint_add_udp` entries match the host-assigned port (default `30120`) and confirm the firewall/security group allows inbound TCP and UDP traffic on that port. Restart txAdmin after adjusting the port so it refreshes the connection.
- **`No such config file: @vMenu/config/permissions.cfg`**: Leave the include commented out in `permission.cfg` unless you have uploaded the default vMenu permissions file.
<<<<<<< codex/build-custom-fivem-server-scripts-yu6dsk
=======
=======
>>>>>>> main
